...
class local_intelliboard_external extends external_api {
    ...
    public function report1($params)
    {
        ...
        $sql_filter .= $this->vendor_filter('ue.userid', $params);
    ...
    public function report2($params)
    {
        ...
        $sql_vendor_filter = $this->vendor_filter('ue.userid', $params);
        $sql_vendor_filter1 = $this->vendor_filter('userid', $params);
        $sql_filter_tracking = '';
    ...
    public function report4($params)
    {
        ...
        $sql_filter .= $this->vendor_filter('u.id', $params);
        $sql_having = $this->get_filter_sql($params, $columns);
        $sql_order = $this->get_order_sql($params, $columns);
        $sql_columns = $this->get_columns($params, ["u.id"]);
        $grade_avg = intelliboard_grade_sql(true, $params);
    ...
    public function report9($params)
    {
        ...
        $sql_filter .= $this->get_filter_course_sql($params, "c.");
        $sql_filter .= $this->vendor_filter('qa.userid', $params);
    ...
    public function report10($params)
    {
        global $CFG;
        $columns = array_merge(array(
            "name","firstname","lastname", "email", "fullname", "qa.state", "qa.timestart", "qa.timefinish", "duration", "grade", "tags", "", "cohortname"
        ), $this->get_filter_columns($params));
    ...
    public function report12($params)
    {
        ...
        $assignment_date_filter = $this->get_filterdate_sql($params, "ra.timemodified");
        $grade_date_filter = $this->get_filterdate_sql($params, "g.timemodified");
        $sql_vendor_filter = $this->vendor_filter('ra.userid', $params);
        $sql_vendor_filter1 = $this->vendor_filter('g.userid', $params);
        $sql_vendor_filter2 = $this->vendor_filter('cc.userid', $params);
        $sql_vendor_filter3 = $this->vendor_filter('lit.userid', $params);
        ...
        if($params->sizemode){
            ...
            $sql_columns = ", MAX(lit.timespend) AS timespend, MAX(lit.visits) AS visits";
            $sql_join = "LEFT JOIN (SELECT lit.userid, lit.courseid,
                                     ...
                                     WHERE lil.id>0 {$date_filter} {$sql_vendor_filter3}
                                  GROUP BY lit.courseid, lit.userid
                                   ) lit ON lit.courseid = c.id AND lit.userid = ra.userid";
        }
        $sql_columns .= $this->get_columns($params, [null]);
        ...
        return $this->get_report_data(
          ...
          LEFT JOIN {role_assignments} AS ra ON ra.contextid = ctx.id {$assignment_date_filter} {$role_filter} {$sql_vendor_filter}
          LEFT JOIN {grade_items} gi ON gi.courseid = c.id AND gi.itemtype = 'course'
          LEFT JOIN {grade_grades} g ON g.itemid = gi.id {$grade_date_filter} {$sql_vendor_filter1}
          LEFT JOIN {course_completions} cc ON cc.course = c.id AND cc.timecompleted > 0 {$completion_date_filter} {$sql_vendor_filter2}
                    {$sql_join}
    ...
    public function report20($params)
    {
        ...
        $sql_filter .= $this->get_filter_course_sql($params, "c.");
        $sql_filter .= $this->vendor_filter('sst.userid', $params);
        $sql_vendor_filter1 = $this->vendor_filter('lit.userid', $params);
        $sql_vendor_filter2 = $this->vendor_filter('userid', $params);
    ...
    public function report21($params)
    {
        ...
        $sql_filter .= $this->vendor_filter('t.userid', $params);
        $grade_single = intelliboard_grade_sql(false, $params);
    ...
    {
        $columns = array_merge(array(
            "r.name", "c.fullname", "visits", "timespend", "r.timemodified"
        ), $this->get_filter_columns($params));
    ...
    public function report42($params)
    {
        ...
        return $this->get_report_data(
            "SELECT CONCAT(ra.userid, '_', c.id) AS id,
                    ...
                    u.email,
                    ra.userid,
                    MAX(ra.timemodified) AS started,
                    c.id AS cid,
                    c.fullname,
                    c.startdate,
                    git.average,
          ...
          LEFT JOIN (SELECT cmc.userid, cm.course, COUNT(cmc.id) as completed
              ...
              WHERE u.id > 0 {$sql_filter}
           GROUP BY ra.userid, ctx.instanceid,cri.gradepass, git.average, u.email, c.id ,cmc.completed, u.firstname,
                    u.lastname, lit.timespend, lit.visits, c.enablecompletion, cc.timecompleted
                    {$sql_having}
                    {$sql_order}",
            $params
        );
    ...
    public function report79($params)
    {
        ...
        $sql_filter .= $this->get_filter_course_sql($params, "c.");
        $sql_filter .= $this->get_filter_module_sql($params, "cm.");
        $sql_vendor_filter = $this->vendor_filter('u.id', $params);
    ...
    public function report97($params)
    {
        ...
        $sql_filter = $this->get_teacher_sql($params, ["c.id" => "courses"]);
        $sql_vendor_filter = $this->vendor_filter('ue.userid', $params);
        $sql_vendor_filter1 = $this->vendor_filter('t.userid', $params);
        $sql_filter .= $this->get_filter_in_sql($params->courseid,'c.id');
        $sql_filter .= $this->get_filter_course_sql($params, "c.");
        ...
        return $this->get_report_data("
            ...
            FROM {course} c
            LEFT JOIN {course_categories} ca ON ca.id = c.category
            LEFT JOIN (SELECT course, COUNT(*) AS completed FROM {course_completions} WHERE timecompleted > 0 GROUP BY course) cc ON cc.course = c.id
    ...
    function report111($params)
    {
        ...
        $sql_filter .= $this->get_filter_user_sql($params, "u.");
        $sql_filter .= $this->get_filter_course_sql($params, "c.");
        $sql_filter .= $this->vendor_filter('lit.userid', $params);
        $sql_join = $this->get_suspended_sql($params);
    ...
    function report141($params)
    {
        ...
        $data = $this->get_report_data(
            "SELECT CONCAT(u.id,'_',c.id) AS id,
               ...
               FROM {course} c
               JOIN {context} ctx ON ctx.instanceid = c.id AND ctx.contextlevel = 50
               JOIN (SELECT ra.contextid, ra.userid
                       FROM {role_assignments} ra
                      WHERE ra.id > 0 {$student_filter_roles}
                   GROUP BY ra.contextid, ra.userid
                    ) rac ON rac.contextid = ctx.id
               JOIN {user} u ON rac.userid = u.id
          ...
          LEFT JOIN {attendance_statuses} stg ON stg.id = atl.statusid AND stg.deleted = 0 AND
    ...
    function report142($params)
    {
        ...
        return $this->get_report_data("
                        ...
                        FROM {course} c
                          ...
                          LEFT JOIN (SELECT cm.course, COUNT(cm.id) AS modules FROM {course_modules} cm WHERE cm.visible = 1 AND cm.completion > 0 GROUP BY cm.course) m ON m.course = c.id
                        ...
                        WHERE c.id > 0 $sql_filter {$sql_vendor_filter} GROUP BY id, userid, u.firstname, u.lastname, u.email, cohorts, ue.timecreated,
    ...
    function report143($params)
    {
        ...
        $columns = array_merge(array(
            "u.firstname","u.lastname","u.email","lit.firstaccess","u.idnumber"
        ), $this->get_filter_columns($params));
        $modules = $this->get_course_modules($params);
        $sql_select = '';
    ...
    function report155($params)
    {
        ...
        $sql_filter .= $this->get_filterdate_sql($params, "ra.timemodified");
        $sql_vendor_filter = $this->vendor_filter('ra.userid', $params);
        $sql_vendor_filter1 = $this->vendor_filter('cmc.userid', $params);
        $sql_join = $this->get_suspended_sql($params);
    ...
    function report156($params)
    {
        ...
        $columns = array_merge(array(
            "ra.id", "c.fullname", "c.shortname", "u.email", "user_name"
        ), $this->get_filter_columns($params));
        $modules = $this->get_course_modules($params);
        $sql_select = '';
        foreach($modules['modules'] as $module){
    ...
    function report164($params)
    {
        global $CFG;
        $columns = array_merge(array(
            "c.fullname","u.firstname","u.lastname","u.email","u.username"
        ), $this->get_filter_columns($params));
        $modules = $this->get_course_modules($params);
        $sql_select = '';
        $columns[] = "cohort_name";
    ...
    public function report182($params)
    {
        ...
        $data = $this->get_report_data(
          ...
          LEFT JOIN {questionnaire_question} qaq ON qaq.surveyid = qa.id AND
                                                       qaq.deleted = 'n'
    ...
    public function report185($params)
    {
        ...
        $columns = array("c.fullname", "q.name", "ta_course.tags", "ta_module.tags", "qs.slot", "q.qtype", "q.name", "qs.s", "qs.facility", "qs.sd", "qs.randomguessscore", "qs.maxmark", "qs.effectiveweight", "qs.discriminationindex", "qs.discriminativeefficiency");
    ...
    public function report188($params) {
        ...
        return $this->get_report_data(
          ...
          LEFT JOIN (SELECT il.checkoutid, {$coursesnames} AS courses
               ...
               WHERE ch.id > 0 {$sql_having} {$sql_order}",
            $params
        );
    }
    ...
    function report195($params)
    {
      ...
      if (!$params->courseid or !is_numeric($params->courseid)) {
        return [];
      } else {
        $this->params['course'] = (int) $params->courseid;
      }
      ...
      if ($modules = $DB->get_records_sql("SELECT id FROM {course_modules} WHERE completion = 1 AND course = :course $sql_mod", $this->params)) {
        foreach($modules as $module) {
            $completion = $this->get_completion($params, "");
      ...
      $data = $this->get_report_data("
        ...
        $completions = $DB->get_records_sql("
              ...
              FROM {course_modules_completion} cmc, {course_modules} cm
              WHERE cm.course = :course AND cmc.coursemoduleid = cm.id $sql_totals GROUP BY cmc.coursemoduleid", $this->params);
    ...
    function report200($params)
    {
        ...
        $sql_filter .= $this->get_filter_in_sql($params->cohortid, "c.id");
        $sql_filter .= $this->get_filter_course_sql($params, "co.");
        $sql_filter .= $this->get_filter_enrol_sql($params, "e.");
        $sql_filter .= $this->get_filter_enrol_sql($params, "ue.");
    ...
    function report201($params)
    {
      ...
      $sql_cohort = $this->get_filter_in_sql($params->cohortid, "cm.cohortid", false);
      ...
      return $this->get_report_data(
        ...
        LEFT JOIN (
          SELECT
            ...
            SUM(CASE WHEN v.value = '6' THEN 1 ELSE 0 END) as value_6,
            SUM(CASE WHEN v.value = '7' THEN 1 ELSE 0 END) as value_7,
            SUM(CASE WHEN v.value = '8' THEN 1 ELSE 0 END) as value_8,
            SUM(CASE WHEN v.value = '9' THEN 1 ELSE 0 END) as value_9,
            SUM(CASE WHEN v.value = '10' THEN 1 ELSE 0 END) as value_10
          FROM
            {feedback_value} v,
            {feedback_completed} c
          WHERE v.completed = c.id AND c.userid IN (SELECT userid FROM {cohort_members} cm WHERE $sql_cohort) GROUP BY v.item
        ) v ON v.item = i.id
    ...
    public function analytic1($params){
        ...
        $sql_vendor_filter = $this->vendor_filter('log.userid', $params);
    ...
    public function analytic3($params){
        global $DB, $CFG;
        $data = array();
        $sql_vendor_filter = $this->vendor_filter('qa.userid', $params);
        $sql_vendor_filter1 = $this->vendor_filter('g.userid', $params);
        if(is_numeric($params->custom)){
    ...
    public function analytic5table($params){
        ...
        $order_sql = $this->get_order_sql($params, $columns);
        $join_sql = $this->get_suspended_sql($params, 'q.course', 'qa.userid');
        $sql_vendor_filter = $this->vendor_filter('userid', $params);
        $params->custom = clean_param($params->custom,PARAM_INT);
        $this->params['custom1'] = $params->custom;
        $this->params['custom2'] = $params->custom;
        $this->params['custom3'] = $params->custom;
        ...
        $sql = "SELECT MIN(qas.id) as id,
                ...
                FROM {quiz} q
                 JOIN (SELECT userid, MAX(attempt) AS attempt, quiz
                       FROM {quiz_attempts}
                        WHERE quiz=:custom1 AND userid != 2 {$sql_vendor_filter} GROUP BY userid,quiz ) AS max_att ON max_att.quiz=q.id
                 JOIN (SELECT userid, MIN(attempt) AS attempt
                       FROM {quiz_attempts}
                        WHERE quiz=:custom2 AND userid != 2 {$sql_vendor_filter} GROUP BY userid ) AS min_att ON max_att.userid=min_att.userid
    ...
    public function analytic8($params){
        ...
        $sql_filter .= $this->vendor_filter('ra.userid', $params);
    ...
    public function analytic9($params){
        ...
        $sql_vendor_filter = $this->vendor_filter('ue.userid', $params);
        $sql_vendor_filter1 = $this->vendor_filter('l.userid', $params);
        $sql_vendor_filter2 = $this->vendor_filter('cc.userid', $params);
    ...
    public function analytic11Table($params){
        ...
        $columns = [
            'course', 'session', 'clbr.timestart', 'clbr.timeend', 'participant',
            'role', 'join_time', 'left_time', 'duration', 'rejoins'
        ];
        $sql_order = $this->get_order_sql($params, $columns);
        $where = 'libp.id > 0';
        ...
        $sql = "SELECT {$randomnumber} frst, c.fullname as course,
                       ...
                       CONCAT(
                           FORMAT(gg.finalgrade, 1), '/', FORMAT(gg.rawgrademax, 1)
    ...
    public function report175($params) {
        ...
        $data = $this->get_report_data(
          ...
          LEFT JOIN (SELECT cx.instanceid as courseid,
                            GROUP_CONCAT(
                                CONCAT(u.firstname, ' ', u.lastname) SEPARATOR ', '
                            ) as teachers
                       FROM {context} cx
          ...
          LEFT JOIN (SELECT cx.instanceid as crs, COUNT(ra.id) particip_num
           ...
           GROUP BY clb.sessionuid, c.id, c.fullname, tcrs.teachers, clb.name, prtcp.particip_num
                    {$sql_having}
                    {$sql_order}",
            $params, false
        );
    ...
    public function report176($params) {
        ...
        $data = $this->get_report_data(
            "SELECT c.id as courseid, c.fullname as course, tcrs.teachers,
                    COUNT(DISTINCT clb.sessionuid) as num_sessions, prtcp.particip_num,
                    SUM(CASE WHEN libp.first_join_time <= clb.timestart THEN 1 ELSE 0 END) on_time,
                    SUM(CASE WHEN libp.first_join_time > clb.timestart AND
                                  libp.first_join_time <= clb.timeend THEN 1 ELSE 0 END) late,
                    prtcp.particip_num * COUNT(DISTINCT clb.sessionuid) - (
          ...
          LEFT JOIN (SELECT cx.instanceid as crs, COUNT(ra.id) particip_num
              ...
              WHERE c.id > 0 {$sql_filter}
           GROUP BY c.id, c.fullname, tcrs.teachers, prtcp.particip_num
        ...
        $sql_date_filter = $this->get_filterdate_sql($params, "clb.timestart");
        $comparedata = $this->get_report_data(
            "SELECT c.id as courseid, prtcp.particip_num,
                    SUM(CASE WHEN libp.first_join_time <= clb.timestart THEN 1 ELSE 0 END) on_time,
                    SUM(CASE WHEN libp.first_join_time > clb.timestart AND
                                  libp.first_join_time <= clb.timeend THEN 1 ELSE 0 END) late,
                    prtcp.particip_num * COUNT(DISTINCT clb.sessionuid) - (
          ...
          LEFT JOIN (SELECT cx.instanceid as crs, COUNT(ra.id) particip_num
              ...
              WHERE c.id > 0
           GROUP BY c.id, prtcp.particip_num",
            $params, false
        );
    ...
    public function report177Chart($params) {
        ...
        if ($CFG->dbtype == 'pgsql') {
            $tsformat = "TO_CHAR(to_timestamp(cb.timestart), 'dd Mon, YYYY')";
        } else {
            $tsformat = "FROM_UNIXTIME(cb.timestart, '%d %b, %Y')";
        }
    ...
    public function report177Table($params) {
        ...
        $sql_filter = $this->get_filter_course_sql($params, "c.");
        $sql_filter .= $this->get_filter_in_sql($params->courseid, 'c.id');
        $rolefilter = $this->get_filter_in_sql($studentroles, 'ra.roleid');
        $sql_date_filter = $this->get_filterdate_sql($params, "clb.timestart");
        $sql_limit = $this->get_limit_sql($params);
        ...
        return $this->get_report_data(
          ...
          LEFT JOIN {local_intelliboard_bb_partic} as libp ON libp.sessionuid = clb.sessionuid AND
           ...
           GROUP BY t.userid, u.firstname, u.lastname {$sql_order} {$sql_limit}",
            $this->params
        );
    }
    ...
    public function report178Charts($params) {
        ...
        $numbers = $DB->get_record_sql(
            "SELECT AVG(t1.sessions_attended) avg_attendned, AVG(t1.sessions_missed) avg_missed
    ...
    public function report178Table($params) {
        $columns = [
            ...
            'libp.last_left_time', 'libp.duration'
        ];
        $user_filter = $this->get_filter_in_sql($params->userid, "u.id", false);
    ...
    function report180SessionDetails($params) {
        ...
        if($data) {
            ...
            $recordings = $DB->get_records(
                'local_intelliboard_bb_rec', ['sessionuid' => $data->sessionuid]
            );
    ...
    public function get_enrollments_per_course($params){
        ...
        return $DB->get_records_sql("
            ...
            FROM {course} c
                ...
                LEFT JOIN {user_enrolments} ue ON ue.enrolid = e.id
                LEFT JOIN {course_completions} cc ON cc.course = e.courseid AND cc.userid = ue.userid AND cc.timecompleted > 0
            WHERE c.id > 0 $sql GROUP BY c.id", $this->params, 0, 100); // maximum
    }
        ...
        if($course->id){
            ...
            $course->mods = $DB->get_records_sql(
                "SELECT MAX(m.id) as id, MAX(m.name) as name, COUNT( cm.id ) AS size
                   FROM {course_modules} cm, {modules} m
                  WHERE cm.visible = 1 AND m.id = cm.module AND cm.course = :course
               GROUP BY cm.module",
               ['course' => $course->id]
            );
    ...
    public function monitor62($params) {
        ...
        $sql = "SELECT floor(lg.timecreated / 86400) * 86400 AS timepoint, COUNT(*) AS all_login,
                       SUM(CASE WHEN origin = 'ws'
                                THEN 1 ELSE 0 END
                       ) AS mobile_app_login
                  FROM {logstore_standard_log} lg
                 WHERE other NOT LIKE '%local_%' AND lg.id > 0 {$sql_filter}
                 GROUP BY timepoint";
    ...
    public function vendor_filter($useridcolumn, $params) {
        global $DB;
        ...
        if(isset($params->vendor_user_id) && $params->vendor_user_id) {
            ...
            if($users) {
                $ids = implode(',', $users);
                return " AND {$useridcolumn} IN ({$ids})";
            }
