...
class local_intelliboard_external extends external_api {
    ...
    {
        $columns = array_merge(array("activity", "m.name", "completed", "visits", "timespend", "grade", "cm.added", "firstaccess", "c.fullname"), $this->get_filter_columns($params));
        ...
        if($params->custom3 == 1){
            if(empty($sql_having) && $params->timestart && $params->timefinish){
                ...
                $sql_having = " HAVING firstaccess BETWEEN :$timestart AND :$timefinish ";
            }else{
                $sql_having .= $this->get_filterdate_sql($params, 'firstaccess');
            }
        }else{
        ...
        return $this->get_report_data("
            SELECT
                ...
                (SELECT MIN(firstaccess) FROM {local_intelliboard_tracking} WHERE page='module' AND param=cm.id) AS firstaccess
                $sql_columns
    ...
    public function report21($params)
    {
        ...
        return $this->get_report_data("
            SELECT t.*,
                ...
                $sql_columns
                FROM (SELECT MIN(id) AS id,userid, scormid, MAX(timemodified) AS timemodified,
                    ...
                    LEFT JOIN (SELECT gi.iteminstance, $grade_single AS score, g.userid
                        FROM {grade_items} gi, {grade_grades} g WHERE gi.itemmodule = 'scorm' and g.itemid = gi.id AND g.finalgrade IS NOT NULL GROUP BY gi.iteminstance, g.userid, g.rawgrademax , g.finalgrade) AS sg ON sg.iteminstance = t.scormid and sg.userid= t.userid
                    $sql_join
    ...
    {
        $columns = array_merge(array("course", "c.shortname","category", "u.firstname", "u.lastname", "email", "e.enrol", "ue.timecreated", "grade", "lastaccess", ""), $this->get_filter_columns($params));
   ...
   function report114_graph($params){
    ...
    public function report165($params)
    {
        ...
        return $this->get_report_data("
            SELECT
                   ...
                   cmc.timemodified AS status_modified,
                   $grade_single AS grade,
                   g.timemodified AS graded,
                   (SELECT DISTINCT CONCAT(u.firstname,' ',u.lastname)
                                FROM {role_assignments} AS ra
    ...
    public function report167($params)
    {
        ...
        return $this->get_report_data("
            ...
            GROUP BY qua.questionid,u.id $sql_having $sql_order", $params);
    }
    ...
    public function report168($params)
    {
        ...
        $items = $DB->get_records('checklist_item', array('checklist'=>$params->custom, 'userid'=>0));
        foreach($items as $item){
            $sql_columns .= ", (SELECT teachertimestamp FROM {checklist_check} WHERE item=$item->id AND userid=u.id AND teachermark=1) AS completed_item_$item->id";
        ...
        $sql_filter = $this->get_filter_in_sql($params->custom, "ch.id", false);
        $sql_filter .= $this->get_filter_user_sql($params, "u.");
        $sql_filter .= $this->get_filter_in_sql($params->learner_roles, "ra.roleid");
        $sql_filter .= $this->get_filter_in_sql($params->cohortid,'coh.id');

        if ($CFG->dbtype == 'pgsql') {
            $group_concat = "string_agg( DISTINCT coh.name, ', ')";
        } else {
            $group_concat = "GROUP_CONCAT(DISTINCT coh.name SEPARATOR ', ')";
        }
        ...
        return $this->get_report_data("
            ...
            FROM {checklist} ch
               ...
               LEFT JOIN {checklist_check} chc ON chc.item=chi.id AND chc.userid=u.id AND chc.teachermark=1
            WHERE $sql_filter
            GROUP BY u.id $sql_having $sql_order", $params);
    }
    ...
    function get_course_checklist_items($params){
        ...
        return $DB->get_records('checklist_item', array('checklist'=>$params->custom, 'userid'=>0), '', 'id,displaytext');
    }
                                ...
                                (SELECT AVG((((q.grade/q.sumgrades)*qa.sumgrades)/q.grade)*100) AS average
                                    FROM {quiz_attempts} qa
                                     ...
                                     JOIN (SELECT userid, MAX(attempt) AS attempt
                                           FROM {quiz_attempts}
                                           WHERE quiz=:custom5 GROUP BY userid ) AS max_att
    ...
    public function get_assign_categories($params)
    {
        ...
        $opr = ($params->custom2) ? "" :"NOT";
        $this->params['userid'] = (int) $params->userid;
        $sql_filter .= " AND id $opr IN(SELECT instance FROM {local_intelliboard_assign} WHERE userid = :userid AND rel = 'external' AND type='categories')";
        $sql_arr = array();
        if ($params->filter) {
